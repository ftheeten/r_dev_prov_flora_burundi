...
#fichier_taxo="C:\\DEV\\salvator\\mar\\20\\taxo_distribution_control_removedup_taxo.txt"
...
#fichier_dist="C:\\DEV\\salvator\\april\\10\\taxo_distribution_control_removedup_remove_sansdist_province_corr.txt"
#fichier_taxo="C:\\DEV\\salvator\\april\\17\\data\\taxo_distribution_control_17april.txt"
fichier_taxo="C:\\DEV\\salvator\\june\\05\\taxo_distribution_control_removedup_taxo20220605.txt"
fichier_syno="C:\\DEV\\salvator\\mar\\20\\export_list_syno_fusionne.txt"
fichier_verna="C:\\DEV\\salvator\\mar\\20\\\\ArticleB_Liste des noms vernaculaires.txt" 
fichier_dist="C:\\DEV\\salvator\\may\\29\\taxo_distribution_control_removedup_provinces.txt"
...
output_file="C:\\Users\\ftheeten\\OneDrive - Africamuseum\\Documents\\salvator\\SALVATOR\\latex\\output_acanthacae_pdf.tex" 
...
def print_distribution(row, distribution_acceptee, file1):
    ...
    for region, a_region in sorted(geo_dict.items()):
        ...
        str_prov="\\emph{"+region + "}" + " - "+"; ".join(list_coll)
        list_sp.append(str_prov)
    str_p="\\small{Spécimens: "+". ".join(list_sp)+"}."
    if go:
...
def write_tex(row, file1, image_file):
    ...
    if 'current_name' in  row:        
        ...
        if "subspecies" in row:
            ...
            for subsp_row in row["subspecies"]:
                write_tex(row["subspecies"][subsp_row], file1, image_file)
        if "variety" in row:
            for var_row in row["variety"]:
                write_tex(row["variety"][var_row], file1, image_file)
        print_distribution(row, distribution_acceptee, file1)
        #include image
        file_exists = exists(image_file)
        if file_exists:
            writeline("\\includegraphics[scale=0.5]{"+image_file.replace("\\","/")+"}", file1, True)
...
def entry_point():
    ...
    global df_verna
    #filtrer acanthacae
    df_taxo=df_taxo[(df_taxo['family']=="Acanthaceae")]
    check_duplicate={}
    try:
        df_taxo['NOM_ACTUEL_A_UTILISER'] = df_taxo['NOM_ACTUEL_A_UTILISER'].str.strip()
        ...
        for index, row in def_species.iterrows():    
            ...
            if(len(species)>0):
                ...
                #    print(clade)
                #    print(family)
                #    print(output_data[clade][family][species])                
        ...
        for index, row in def_subspecies.iterrows():    
            ...
            if(len(subsp)>0):
                ...
                if not species_name in output_data[clade][family]:
                    output_data[clade][family][species_name]={"remark":"Only present as subspecies or variety", "current_name":species_name, "rank":"Species_or_higher"}
                else:    
        ...
        for clade, nested in output_data.items():           
            ...
            for current_family, nested2 in nested.items():
                ...
                for species, nested3 in nested2.items():
                    ...
                    print(image_file)
                    write_tex(nested3, file1, image_file)
